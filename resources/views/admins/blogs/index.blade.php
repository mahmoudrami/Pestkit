...
@section('content')
    ...
    <div class="d-flex justify-content-between mb-3">
        ...
        <a href="{{ route('admin.blogs.create') }}" class="btn btn-secondary mr-5 h-75 p-2">add new</a>
    </div>
    @if (session('msg'))
    ...
    <table class="table table-hover table-bordderd ">
        ...
        @foreach (@$items as $item)
            <tr>
                ...
                <th>{{ @$item->title }}</th>
                <th>{{ @$item->comments_count }}</th>
                <th>
                    <a href="{{ route('admin.blogs.edit', @$item->id) }}" class="btn btn-primary"><i class="fas fa-edit"></i></a>
